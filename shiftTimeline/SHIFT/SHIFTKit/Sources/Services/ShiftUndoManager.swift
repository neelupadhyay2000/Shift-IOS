...
public struct BlockSnapshot: Codable, Sendable, Equatable {
    ...
    /// Whether the block had an unresolved collision at snapshot time.
    ///
    /// `CollisionDetector` and `CompressionCalculator` both mutate this flag
    /// during the engine pipeline. Including it here ensures undo/redo restores
    /// the full engine-mutated state rather than leaving stale collision markers.
...
/// ```swift
/// // Before the ripple:
/// undoManager.recordShift(blocks: allBlocks)
///
/// // Run the engine (mutates blocks in place)…
///
/// // After the ripple:
/// undoManager.commitShift(blocks: allBlocks)
///
/// // In your undo action:
/// if let snapshots = undoManager.undo() {
///     apply(snapshots, to: allBlocks)
/// }
/// ```
///
...
public final class ShiftUndoManager {
    ...
    // MARK: - Undo / Redo — snapshot-returning overloads
    ...
    // MARK: - Undo / Redo — convenience applying overloads
