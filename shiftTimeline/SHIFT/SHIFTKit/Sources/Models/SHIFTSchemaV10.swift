...
/// V10 schema — adds to `EventModel`:
///   * `lastShiftedAt: Date?` — CloudKit "parent tickle" stamped on every
///     timeline shift so `NSPersistentCloudKitContainer` exports a parent-record
...
///
/// **Critical:** Re-declares all `@Model` types in full so V10 has a distinct
/// schema checksum from V9; reusing live types would collapse both versions
/// onto the same checksum and cause `MigrationStage.lightweight(V9 → V10)`
/// to throw an `NSException`.
public enum SHIFTSchemaV10: VersionedSchema {
    public static var versionIdentifier: Schema.Version { Schema.Version(10, 0, 0) }

    public static var models: [any PersistentModel.Type] {
        [
            SHIFTSchemaV10.EventModel.self,
            SHIFTSchemaV10.TimeBlockModel.self,
            SHIFTSchemaV10.TimelineTrack.self,
            SHIFTSchemaV10.VendorModel.self,
            SHIFTSchemaV10.ShiftRecord.self,
        ]
    }
    ...
    public final class EventModel {
        ...
        /// New in V10. Parent-tickle timestamp updated on every shift.
