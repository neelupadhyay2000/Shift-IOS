...
struct ActiveBlockHeroOvertimeTests {
    ...
    @Test func blockEndedInPastIsOvertime() {
        let now = Date.now
        let start = now.addingTimeInterval(-120) // started 2 min ago
    ...
    @Test func blockEndingInFutureIsNotOvertime() {
        let now = Date.now
        let start = now.addingTimeInterval(-30)  // started 30s ago
    ...
    @Test func blockEndingExactlyNowIsOvertime() {
        let now = Date.now
        // Nudge 1s into the past to guarantee remaining < 0 in test execution.
        let duration: TimeInterval = 60
        let start = now.addingTimeInterval(-duration - 1)
        let block = TimeBlockModel(
            title: "Setup",
            scheduledStart: start,
            duration: duration
    ...
    @Test func firstBlockGetsActiveStatus() throws {
        let blocks = [
        ...
        #expect(!blocks.contains(where: { $0.status == .active }),
                "precondition: no block should be active before activation")
        let first = try #require(blocks.first(where: { $0.status != .completed }),
                                "precondition: at least one incomplete block must exist")
